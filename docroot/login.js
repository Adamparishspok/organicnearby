...
function filterTimezoneSelect() {
  ...
  if (am.checked == false) {
    // PM times
    // cast to int
    if (hour < 12) hour = hour - 0 + 12;
  } else {
    // AM times, this should only happen for 12 AM
    if (hour > 11) hour = hour - 0 - 12;
  }
...
function incrementHour(a, h) {
  ...
  if (beforehour == 11 && hour == 12) switchAmPm();
  else if (beforehour == 12 && hour == 11) switchAmPm();
...
function checkPasswordRules() {
  ...
  if (p1.value.length < 8) {
    ...
    success = false;
  } else req1.classList.add("loginGreyed");
